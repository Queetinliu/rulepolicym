...
/*
	// do a request
	client := &http.Client{}
	req, err := http.NewRequest("POST", Sso_Url, nil)
	if err != nil {
		return SsoResp{}, fmt.Errorf("create new request to %s with err:%w", Sso_Url, err)

	}
	req.Header.Set("Authorization", fmt.Sprintf("Basic %s", userpassbase64))
	resp, err := client.Do(req)
	if err != nil {
		return SsoResp{}, fmt.Errorf("do request to %s ,body: %s with err:%w", Sso_Url, req.Body, err)
	}
	...
	var ssoResp SsoResp
	err = json.Unmarshal(bodyText, &ssoResp)
	if err != nil {
		return SsoResp{}, err
	}
	return ssoResp, nil
...
/*
	...
	client := &http.Client{}
	req, err := http.NewRequest("POST", "http://30.1.64.241/dce/auth/account", nil)
	if err != nil {
		return AuthResp{}, fmt.Errorf("create new request to %s with err:%w", "http://30.1.64.241/dce/auth/account", err)
	}
	req.Header.Set("X_DCE_ACCESS_TOKEN", access_token)
	resp, err := client.Do(req)
	if err != nil {
		return AuthResp{}, fmt.Errorf("do request to %s with err:%w", "http://30.1.64.241/dce/auth/account", err)
	}
	defer resp.Body.Close()
	bodyText, err := io.ReadAll(resp.Body)
	if err != nil {
